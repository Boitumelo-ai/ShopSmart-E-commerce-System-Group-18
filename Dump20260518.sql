...
--
-- Host: localhost    Database: shopsmart
-- ------------------------------------------------------
...
INSERT INTO `category` VALUES (1,'Electronics'),(2,'Clothing'),(3,'Food');
...
CREATE TABLE `review` (
  ...
  KEY `user_id` (`user_id`),
  KEY `product_id` (`product_id`),
  CONSTRAINT `review_ibfk_1` FOREIGN KEY (`order_id`) REFERENCES `orders` (`id`),
...
INSERT INTO `users` VALUES (1,'John','Smith','john@gmail.com','12345',1),(2,'Mary','Jones','mary@gmail.com','12345',2);
...
/*!50001 VIEW `customer_orders` AS select `users`.`first_name` AS `first_name`,`users`.`last_name` AS `last_name`,`orders`.`id` AS `order_id`,`orders`.`total_amount` AS `total_amount`,`orders`.`status` AS `status` from (`users` join `orders` on((`users`.`id` = `orders`.`user_id`))) */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `product_reviews` AS select `product`.`description` AS `description`,`review`.`comment` AS `comment`,`review`.`rating` AS `rating` from (`product` join `review` on((`product`.`id` = `review`.`product_id`))) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2026-05-18 23:48:03
